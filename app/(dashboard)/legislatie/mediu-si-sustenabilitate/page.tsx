// app/legislatie/mediu-si-sustenabilitate/page.tsx
'use client';

import Link from 'next/link';
import { useRouter } from 'next/navigation';
import s from '../legislatie.module.css';
import { LEGI_MEDIU_SUSTENABILITATE } from './data';

const fmt = (d?: string) => (d ? new Date(d).toLocaleDateString('ro-RO') : '—');

export default function MediuSiSustenabilitate() {
    const r = useRouter();
    ...
    return (
        <div className={s.page}>
            <div className={s.header}>
                ...
                <h1 className={s.h1}>Mediu și sustenabilitate</h1>
            </div>

            <div className={s.grid} style={{ display: 'grid' }}>
                {LEGI_MEDIU_SUSTENABILITATE.map((lege) => (
                    <div
                        key={lege.slug}
                        className={s.card}
