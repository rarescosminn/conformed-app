...
type Props = { onboardingDoneDB: boolean };

export default function SettingsPage({ onboardingDoneDB }: Props) {
    const HOSPITAL_ID = getHospitalId();
    const { orgType } = useOrg();
    const [onboardingDone, setOnboardingDone] = useState(onboardingDoneDB);
